...
def compute_metrics(metric:Callable,lables:np.ndarray,p: EvalPrediction):
    ...
    result = metric.compute(predictions=preds, references=p.label_ids)  #计算accuracy
    result["micro_f1"] = f1_score(
        y_true=preds, y_pred=p.label_ids, average="micro", sample_weight=None,labels=lables
    )
...
def main():
    ...
    # use "rotten_tomatoes" as the name of this adapter
